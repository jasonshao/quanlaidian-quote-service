    ...
    for item in config.get("报价项目", []):
        ...
        items.append(
            QuoteItemBreakdown(
                ...
                unit_price=float(item.get("商品单价", 0)),
                subtotal=float(item.get("报价小计", 0)),
                cost_unit_price=float(item.get("成本单价", 0)),
                cost_subtotal=float(item.get("成本小计", 0)),
                profit=float(item.get("利润", 0)),
