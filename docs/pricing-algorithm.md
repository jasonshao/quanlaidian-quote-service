...
1–30 店的平滑曲线外推到大段会在 ~90 店变负，数学崩溃，且业务上"大客户"是离散谈判（50/100/200/300 是常见合同规模），所以大段**不做连续曲线**，直接用锚点：

```python
LARGE_SEGMENT_ANCHORS = {
    "轻餐": [(50, 0.15), (100, 0.13), (200, 0.12), (300, 0.11)],
    "正餐": [(50, 0.18), (100, 0.16), (200, 0.14), (300, 0.13)],
}
```

这些值是业务经验值，**调整策略**：首发后主要靠销售在 Excel 里手改，系统版本每 1–2 季度 review 一次，如需改动重新部署。

### 5.2 路由：大客户段强制阶梯

当 `31 ≤ 门店数量 ≤ 300` 时：

1. `determine_route_strategy` 返回 `"large-segment"`。
2. `resolve_tier_window(n)` 计算覆盖 n 的**相邻两档**锚点：
...
3. 主 `报价项目` 按 `tier_window[0]`（下锚点）的门店数 + 因子生成 —— 即给客户看"下锚点方案"作为默认。
4. `阶梯配置` 包含完整两档数据，PDF/Excel 的阶梯对比页渲染 `[tier_window[0], tier_window[1]]` 两列。
5. `original_requested_store_count` 保留在 `pricing_info` 里供审计。
6. `是否启用阶梯报价` 标志**被忽略**：大段永远给阶梯。

### 5.3 301 店及以上

`recommend_base_deal_price_factor_smooth` 和 schema `le=300` 都会拒绝：
...
大客户定价需人工评估后用 legacy 通道。

### 5.4 小段（1–30）与阶梯对比的关系

小段行为**不变**：

- `是否启用阶梯报价=false`：单点报价（主 `报价项目`，无 `阶梯配置`）。
- `是否启用阶梯报价=true`：单点报价 + 附 `[10, 20, 30]` 阶梯对比页。

小段的 factor 仍走 §2 的平滑曲线，tier [10, 20, 30] 的每档因子用同一公式算，不涉及大段锚点。
